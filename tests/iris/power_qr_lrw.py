...
print("Power method with QR for L_rw matrix:")
for k in range(n):
    start = time.time()

    H, Q = qr_iteration_householder_shift_deflation(L_rw, max_iter=1000, tol=1e-3)
    diag = np.diag(H)

    results = Parallel(n_jobs=-1)(
        delayed(shifted_inverse_power_method)(L_rw, Q[i, :], diag[i])
